package com.xiaoyi.mall.thirdparty.template.impl;
...
import com.xiaoyi.mall.thirdparty.entity.OssFile;
import com.xiaoyi.mall.thirdparty.template.OssTemplate;
import io.minio.*;
...
public class MinioTemplate implements OssTemplate {
	...
	public OssFile upLoadFile(String folderName, String fileName, MultipartFile file) {
		if (file == null || file.isEmpty()) {
			throw new RuntimeException("400", new Throwable("上传文件不能为空"));
		}
		// 文件大小
		if (file.getSize() > 5 * 1024 * 1024) {
			throw new RuntimeException("400", new Throwable("文件大小不能超过5M"));
		}
		...
		if (!CollUtil.contains(ossProperties.getFileExt(), suffix)) {
			...
			throw new RuntimeException("400", new Throwable(error));
		}
		try {
			return upLoadFile(folderName, fileName, suffix, file.getInputStream());
		} catch (Exception e) {
			logger.error("minio upLoadFile Exception:{}", e);
			throw new RuntimeException("400", new Throwable("文件上传失败,请重新上传或联系管理员"));
		}
	}
